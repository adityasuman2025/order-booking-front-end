...
//rendering
	return(
		<nav className="navbar navbar-expand-md navbar-dark fixed-top bg-dark noPadding" >
			...
			<button id="mobileNavBarToggler" onClick={ handleMobileNavBarToggler } className="navbar-toggler" >
				<span className="navbar-toggler-icon"></span>
			</button>

			<div
				id="navbarCollapse"
				className={ classNames( 'navbar-collapse', { 'collapse': !mobileNavBarToggler }, { 'display':mobileNavBarToggler } ) }
				// className={ this.state.mobileNavBarToggler === false ? "collapse " : "display" }
			>
				<ul className="navbar-nav mr-auto navWebMenu" >
					<li className={ classNames( "nav-item", { "active": props.active == "dashboard" } ) }>
						<Link className="nav-link" to="/admin/dashboard">Dashboard</Link>
					</li>
					<li className={ classNames( "nav-item", { "active": props.active == "filter" } ) }>
						<Link className="nav-link"  to="/admin/filter">Filter</Link>
					</li>
					<li className={ classNames( "nav-item", { "active": props.active == "create_product" } ) }>
