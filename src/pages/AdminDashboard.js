...
//function to style graph pillar as per its order count and position
    const graphPillarStyle = ( max_count, count, idx ) => {
        ...
        const pillarHeight = Math.floor( ( graphBox_Height * count ) / max_count ) || 17; // min height of 17 will be there in case of max_count = 0
                                        ...
                                        return(
                                            <div key={ idx } className="graphPillarBox">
                                                <div 
                                                    ...
                                                    style={ graphPillarStyle( todaysTopMaxOrderCount, item.order_count, idx ) }>
                                                    { item.order_count }
                                                </div>
                                    ...
                                    todaysBottom3.map( ( item, idx ) => {
                                        return(
                                            <div key={ idx } className="graphPillarBox">
                                                <div 
                                                    className="graphPillar" 
                                                    style={ graphPillarStyle( todaysBottomMaxOrderCount, item.order_count, idx ) }>
                                                    { item.order_count }
                                                </div>
                                                <span className="graphPillarTitle">{ item.city_name }</span>
                                            </div>
                                    ...
                                    weeklyTop3.map( ( item, idx ) => {
                                        return(
                                            <div key={ idx } className="graphPillarBox">
                                                <div 
                                                    className="graphPillar" 
                                                    style={ graphPillarStyle( weeklyTopMaxOrderCount, item.order_count, idx ) }>
                                                    { item.order_count }
                                                </div>
                                                <span className="graphPillarTitle">{ item.city_name }</span>
                                            </div>
                                    ...
                                    weeklyBottom3.map( ( item, idx ) => {
                                        return(
                                            <div key={ idx } className="graphPillarBox">
                                                <div 
                                                    className="graphPillar" 
                                                    style={ graphPillarStyle( weeklyBottomMaxOrderCount, item.order_count, idx ) }>
                                                    { item.order_count }
                                                </div>
                                                <span className="graphPillarTitle">{ item.city_name }</span>
                                            </div>
