...
//componentDidMount
    useEffect( () => {
        ...
        //by default first page of products will be listed
            ...
            const entered_PhoneNo = await getDecryptedCookieValue( "order_booking_user_phone_no" ) || "";
...
//when proceed btn is pressed
    ...
    //checking if some phone no is entered 
        ...
        if( entered_PhoneNo == "" ) {
            ...
            const response = await checkUserExistsWithGivenPhoneNumber( entered_PhoneNo );
            if( response == 1 ) {
            //making of the phone no entered by user and redirecting to book-order page
                const enteredPhoneNo_cookie = await makeEncryptedCookie( "order_booking_user_phone_no", entered_PhoneNo );
                ...
                await makeSnackBar( "This phone number is not registered. Please Create a User to continue", "error" );
            } else {
                await makeSnackBar( "Something went wrong", "error" );
            }
        }
        ...
        <div>
            <div className="pageContent center">
                ...
                <button 
                    ...
                    onClick={ () => { props.history.push( '/user/create-user' ) } }
                >
                    CREATE USER
                </button>
